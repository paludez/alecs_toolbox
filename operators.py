...
class ALEC_OT_boolean_op(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for target in targets:
            mod = active.modifiers.new(name=f"Bool_{self.operation}", type='BOOLEAN')
...
class ALEC_OT_mirror_control(bpy.types.Operator):
    ...
    def modal(self, context, event):
        if event.type == 'MOUSEMOVE':
            ...
            # Update location (World XY)
            self.empty.location.x = self.initial_loc.x + delta_x * sens
            self.empty.location.y = self.initial_loc.y + delta_y * sens
...
class ALEC_OT_load_material_image(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.report({'WARNING'}, "No image found on Base Color")
        return {'CANCELLED'}
