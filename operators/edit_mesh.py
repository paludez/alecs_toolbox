...
_THREE_PT_PICK_RADIUS_PX = 12
_THREE_PT_SNAP_RING_RADIUS_PX = 12.0
...
def _clear_three_pt_pick_preview():
    if draw_state._draw_data.get('three_pt_picked_world') is not None:
...
def _refresh_three_pt_pick_visual(context, obj, picked_indices, mouse_xy):
    draw_state._draw_data['object_name'] = obj.name
    draw_state._draw_data['mesh_name'] = obj.data.name
    mw = obj.matrix_world
...
def _three_pt_pick_status_message(picked_count: int) -> str:
    if picked_count <= 0:
        return '3pt Circle: Click first vertex  [RMB] Cancel  [Esc] Cancel'
    if picked_count == 1:
        return '3pt Circle: Click second vertex  [RMB] Undo  [Esc] Cancel'
    if picked_count == 2:
        return '3pt Circle: Click third vertex  [RMB] Undo  [Esc] Cancel'
    return '3pt Circle'
...
def _three_pt_create_circle_ring(bm, mw, definition_verts, segments, anchor_index, session):
    ...
    phase = emh.circle_phase_for_point(
        center_w, u_axis, v_axis, points_w[anchor_index],
    )
...
def clear_two_point_circle_session():
    ...
    _two_pt_circle_session = None
    _two_pt_pick_session = None
    _clear_two_pt_pick_preview()


def _clear_two_pt_pick_preview():
    draw_state._draw_data.pop('two_pt_circle_preview', None)
    draw_state._draw_data.pop('cursor_plane', None)
    if draw_state._draw_data.get('three_pt_picked_world') is None:
        draw_state._draw_data.pop('draw_snap_ring', None)
        draw_state._draw_data.pop('draw_snap_source_screen', None)
    if not draw_state.has_dim_overlay_data():
        draw_state.refresh_edit_mesh_px_handler(bpy.context)
...
def _two_pt_circle_session_valid(context, session):
    if session is None:
        return False
    obj = context.active_object
    if obj is None or obj.type != 'MESH':
...
def _two_pt_verts_from_session(bm, session):
    indices = session.get('vert_indices')
...
def _two_pt_remove_created_circle(bm, session):
    _three_pt_remove_created_circle(bm, session)


def _two_pt_points_on_cursor_plane(context, mw, v0, v1):
    """World points of two verts projected onto the 3D cursor XY plane."""
...
def _two_pt_create_circle_ring(
    ...
    for i in range(segments):
        try:
            ...
            pass
    return len(session['created_edge_keys']) > 0


def _two_pt_pick_status_message(picked_count: int) -> str:
    if picked_count <= 0:
        return '2pt Circle: Click first vertex  [RMB] Cancel  [Esc] Cancel'
    if picked_count == 1:
        return '2pt Circle: Click second vertex  [RMB] Undo  [Esc] Cancel'
    return '2pt Circle'
...
class ALEC_OT_two_point_circle(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        status_bar.set_message(context, _two_pt_pick_status_message(0))
        ...
        if event.type == 'RIGHTMOUSE' and event.value == 'PRESS':
            ...
            if picked:
                ...
                status_bar.set_message(
                    context, _two_pt_pick_status_message(len(picked)),
                )
                _refresh_three_pt_pick_visual(
    ...
    def check(self, context):
        global _two_pt_circle_session
        if _two_pt_circle_session_valid(context, _two_pt_circle_session):
    ...
    def execute(self, context):
        global _two_pt_circle_session
        if not _two_pt_circle_session_valid(context, _two_pt_circle_session):
        ...
        definition_verts = _two_pt_verts_from_session(bm, session)
        if definition_verts is None:
            self.report({'WARNING'}, "Definition vertices are no longer valid")
            return {'CANCELLED'}

        mw = obj.matrix_world
        p1_w, p2_w, _plane_n = _two_pt_points_on_cursor_plane(
            context, mw, definition_verts[0], definition_verts[1],
        )
        ...
        _two_pt_remove_created_circle(bm, session)

        bisect_sign = float(session.get('bisect_sign', 1.0))

        if not _two_pt_create_circle_ring(
            context,
            bm,
            obj.matrix_world,
            definition_verts,
...
class ALEC_OT_three_point_circle(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        _refresh_three_pt_pick_visual(context, obj, [], self._last_mouse)
        status_bar.set_message(
            context, _three_pt_pick_status_message(0),
        )
        context.window_manager.modal_handler_add(self)
        _tag_view3d_redraw(context)
        return {'RUNNING_MODAL'}
    ...
    def modal(self, context, event):
        ...
        if event.type == 'RIGHTMOUSE' and event.value == 'PRESS':
            ...
            if picked:
                picked.pop()
                status_bar.set_message(
                    context, _three_pt_pick_status_message(len(picked)),
                )
                _refresh_three_pt_pick_visual(
                    context, self._obj, picked, self._last_mouse,
                )
