...
def create_interactive_offset_bbox(context):
    ...
    offset_obj.name = f"{active_obj.name}_offset"
    ...
    orig_coords = [v.co.copy() for v in bm.verts]
    orig_normals = [v.normal.copy() for v in bm.verts]
    bm.free()

    # 3. Setup display
    setup_bbox_visibility(offset_obj, color=(0.0, 0.5, 1.0, 1.0))
    set_shading_to_object(context)
    ...
    return offset_obj, orig_coords, orig_normals, active_obj

def update_interactive_offset_bbox(offset_obj, offset, orig_coords, orig_normals):
    """Updates the vertex positions of the offset object."""
    if not offset_obj or not offset_obj.data:
        return

    mesh = offset_obj.data
    if len(mesh.vertices) != len(orig_coords):
        # The number of vertices in the offset object is different from the original.
        # This can happen if the user changed the mesh while the modal op is running.
        # To handle this, we can try to remap based on the original object.
        # For now, we'll just abort.
        print("Vertex count mismatch. Aborting update.")
        return

    # Calculate new positions by applying offset to original coordinates
    new_coords = [co + normal * offset for co, normal in zip(orig_coords, orig_normals)]
